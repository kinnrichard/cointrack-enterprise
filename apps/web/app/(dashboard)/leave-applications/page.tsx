...
import { CalendarOff, Filter, X, Loader2, Pencil, Trash2, CheckCircle, XCircle } from 'lucide-react';
...
const schema = z.object({
  ...
  days: z.coerce.number().min(0.5, 'At least 0.5 days'),
...
export default function LeaveApplicationsPage() {
  ...
  const [editing, setEditing] = useState<LeaveApp | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<LeaveApp | null>(null);
  ...
  const { register, handleSubmit, reset, control, formState: { errors, isValid } } = useForm<FormData>({
    resolver: zodResolver(schema), defaultValues: { employeeId: '', leaveTypeId: '', startDate: '', endDate: '', days: 1, reason: '' }, mode: 'onChange',
  });
  ...
  const createMutation = useMutation({
    mutationFn: (data: FormData) => api.post('/leaves', data),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['leaves'] }); closeModal(); toast({ title: 'Leave application created' }); },
    onError: () => toast({ title: 'Error', description: 'Failed to create.', variant: 'destructive' }),
  });

  const updateStatusMutation = useMutation({
    mutationFn: ({ id, status }: { id: string; status: string }) => api.put(`/leaves/${id}`, { status }),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['leaves'] }); toast({ title: 'Status updated' }); },
    onError: () => toast({ title: 'Error', description: 'Failed to update.', variant: 'destructive' }),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => api.delete(`/leaves/${id}`),
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['leaves'] }); setDeleteTarget(null); toast({ title: 'Deleted' }); },
    onError: () => toast({ title: 'Error', description: 'Failed to delete.', variant: 'destructive' }),
  });

  function openAdd() { reset({ employeeId: '', leaveTypeId: '', startDate: '', endDate: '', days: 1, reason: '' }); setEditing(null); setModalOpen(true); }
  function closeModal() { setModalOpen(false); setEditing(null); }
  function onSubmit(data: FormData) { createMutation.mutate(data); }
  ...
  const columns: Column<LeaveApp>[] = [
    ...
    { key: 'leaveType.name', label: 'Type', render: (v: string) => v || '-' },
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader title="Leave Applications" description="Manage leave requests and approvals">
        <Button onClick={openAdd} className="bg-gradient-to-r from-red-700 to-red-600 text-white hover:opacity-90">File Leave</Button>
      </PageHeader>
      ...
      <Dialog open={modalOpen} onOpenChange={(open) => !open && closeModal()}>
        <DialogContent className="max-w-lg max-h-[90vh] flex flex-col p-0 gap-0">
          <div className="px-6 pt-5 pb-4 bg-muted/50 border-b rounded-t-2xl">
            <DialogTitle className="text-xl font-semibold">File Leave Application</DialogTitle>
            <DialogDescription className="text-sm text-muted-foreground mt-1">Submit a new leave request for an employee.</DialogDescription>
          </div>
          <div className="flex-1 overflow-y-auto px-6 py-5">
            <form id="leave-form" onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Controller control={control} name="leaveTypeId" render={({ field }) => (
                    <Select value={field.value} onValueChange={field.onChange}>
                      <SelectTrigger className={cn(errors.leaveTypeId && 'border-red-300')}><SelectValue placeholder="Select type" /></SelectTrigger>
                      <SelectContent>{(leaveTypes.data ?? []).map(t => <SelectItem key={t.id} value={t.id}>{t.name}</SelectItem>)}</SelectContent>
                    </Select>
                  )} />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input type="number" step="0.5" {...register('days')} className={cn(errors.days && 'border-red-300')} />
                </div>
              </div>
              <div className="space-y-1.5">
                <Label className="text-sm">Reason</Label>
                <Textarea {...register('reason')} placeholder="Reason for leave..." className="min-h-[80px]" />
              </div>
            </form>
          </div>
          <div className="px-6 py-4 border-t border-border/50 flex items-center justify-between">
            ...
            <Button type="submit" form="leave-form" disabled={!isValid || createMutation.isPending} className="bg-gradient-to-r from-red-700 to-red-600 text-white hover:opacity-90 rounded-lg">
              {createMutation.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Submit Leave
            </Button>
          </div>
        </DialogContent>
      </Dialog>
      ...
      <ConfirmDialog open={!!deleteTarget} onOpenChange={(open) => !open && setDeleteTarget(null)}
        title="Delete Leave Application" description={`Are you sure you want to delete this leave application?`}
