...
import { z } from 'zod';
import { Briefcase, Pencil, Trash2, Users, Loader2, Filter, X } from 'lucide-react';
...
export default function PositionsPage() {
  ...
  let filtered = statusFilter === 'all' ? positions
    : statusFilter === 'active' ? positions.filter((p) => p.isActive)
    : positions.filter((p) => !p.isActive);
  if (filterHasEmployees === 'with') filtered = filtered.filter(p => p._count.employees > 0);
  if (filterHasEmployees === 'without') filtered = filtered.filter(p => p._count.employees === 0);
              ...
              <PopoverContent className="w-[260px] p-0" align="start">
                ...
                <div className="p-4 space-y-4">
                  <div className="space-y-1.5">
                    <Label className="text-[11px] uppercase tracking-wider text-muted-foreground font-medium">Employees</Label>
                    <select value={filterHasEmployees} onChange={(e) => { setFilterHasEmployees(e.target.value); setPage(1); }} className="flex h-9 w-full rounded-lg border border-input bg-background px-3 py-2 text-sm">
                      <option value="all">All positions</option><option value="with">With employees</option><option value="without">Without employees</option>
                    </select>
                  </div>
                </div>
              </PopoverContent>
